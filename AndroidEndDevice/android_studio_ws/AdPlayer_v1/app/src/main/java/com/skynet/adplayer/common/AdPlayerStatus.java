...
public class AdPlayerStatus {
    ...
    protected String lastLoadContentUrl;
    protected boolean connected;
    protected boolean playing;
    protected String checkVersionUrl;
    protected String downloadUrlPrex;

    public String getLastLoadContentUrl() {
        return lastLoadContentUrl;
    }

    public void setLastLoadContentUrl(String lastLoadContentUrl) {
        this.lastLoadContentUrl = lastLoadContentUrl;
    }
    ...
    public boolean needRefresh(String startUpUrl) {
        ...
        if (!isPlaying() && isConnected()){
            ...
            refreshStatus(startUpUrl);
            return true;
        }

        if (isPlaying() && !startUpUrl.equals(getLastLoadContentUrl())){
            refreshStatus(startUpUrl);
            return true;
        }
        if (isPlaying() && !isConnected()){
    ...
    private void refreshStatus(String startUpUrl){
        setLastReloadTime(new Date());
        setPlaying(true);
        setLastDisconnectTime(null);
        setLastLoadContentUrl(startUpUrl);
    }
}
